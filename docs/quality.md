...
- The app's four supported languages were corrected from an unrelated `en/nl/de/fr` set to the
  ...
  audit, not a hardening-only change. 
- Announcements are pre-recorded MP3s rather than synthesized speech, so the voice-selection code
  and its tests were removed ([ADR 0008](adr/0008-single-shared-relay-and-prerecorded-audio.md)).
